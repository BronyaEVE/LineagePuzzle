...
| Global view aggregates all scripts, layered flow | Click an edge to see `target column ← source columns` + transform expressions |
...
| **Tag Filter** | **Search (table = impact analysis)** |
| ![Tag Filter](docs/images/tag-filter.png) | ![Search](docs/images/search.png) |
| Filter the canvas to tagged scripts' lineage slice | Searching a table name triggers bi-color impact highlight |
| **Tag Dimension Config** | **Preprocess Rules** |
| ![Tag Schema](docs/images/tag-schema.png) | ![Preprocess Rules](docs/images/preprocess-rules.png) |
| Admin maintains dimension names + tag values | Regex replacement rules; param mapping is a built-in type |
...
> Full feature docs, architecture, and API reference in **[docs/PROJECT.en.md](docs/PROJECT.en.md)**.
...
├── docs/
│   ├── PROJECT.en.md      # detailed project docs (architecture/API/advanced usage)
│   └── images/            # screenshots
