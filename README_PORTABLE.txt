...
【参数映射 / Param Mapping】
------------------------------------------------------------
  ETL 脚本常用 ${icl_schema}、${batch_date} 等占位符。
  点击 Header「参数映射」按钮配置实际值（如 icl_schema=ods），
  分析时自动替换：${icl_schema}.orders → ods.orders。
  未配置的参数保留参数名当标识符。
  注意：参数映射在分析新脚本时生效，已有脚本需重新分析。

------------------------------------------------------------
【搜索 / Search】
------------------------------------------------------------
  Header 左侧搜索框支持模糊匹配表名和字段名。
  - 输入关键词 → 下拉显示匹配的表（●）和字段（◆/◇）
  - 选中表 → 图聚焦该节点 + 白边框发光
  - 选中字段 → 图聚焦相关边 + 单边高亮 + 弹列级 Drawer
