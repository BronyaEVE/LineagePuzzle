...
>
> **引擎依赖（2026-08 起）**：分析管线（预处理/拆分/AST 提取/列级映射/归一化）来自
> [lineage_puzzle](https://github.com/BronyaEVE/lineage_puzzle) 包（`requirements.txt` 钉 git tag），
> 本仓 `services/` 只保留 Web 侧编排与存储；引擎的源码与测试在引擎仓维护，
> 引擎 bug 修复流程 = 引擎仓改 → 打 tag → 本仓升依赖。
...
| 顶部搜索框输入 | 只搜字段（`表.列`）。选中字段 = 高亮该字段流转经过的所有边（紫色）+ 视口聚焦到相关区域。重复搜同一目标也会重新聚焦 |
| 左栏搜索框输入 | 只搜表名，过滤左侧表列表；点击表项即进入其邻域子图 |

> **折叠：** 每个节点在入边侧/出边侧各有一个 14px 的 +/- 按钮（TB 布局在节点上下边缘，LR 布局在左右边缘）。折叠会递归隐藏整条上游/下游链路。切换脚本或切到全局图谱时折叠状态自动重置。

**高亮优先级：** 单边（点边） > 影响分析（上下游双色） > 字段搜索（紫色） > 默认（无高亮，静态边）

> **边动画：** 默认所有边都是静态的（无流动动画），保证大图浏览流畅——只有被高亮的边才会流动。

**节点样式：** 宽度自适应（150–300px）。带 schema 的表名两行显示——schema 是顶部小字副行，表名独占主行；超长部分截断省略，完整名悬停可见，点击节点展开为单行全名（白边框发光）。
...
- **左栏表列表搜索**：模糊匹配表名（`schema.table`），过滤表列表；点击表项进入邻域子图。表为中心的导航主入口。
- **顶部搜索框（Header）**：只搜字段名。字段按「表.列」聚合，不同表的同名字段是独立条目（`orders.id` 与 `users.id` 分开）；选中字段 → 图高亮**该字段流转经过的所有边**（紫色 `#722ed1`）+ 视口聚焦到这些边的端点区域。跨多边的字段在下拉里显示「(N 条流转)」。
  - 字段命中按 (source, target) 对匹配，全局视图与邻域子图视图都能正确高亮（边 id 编号随视图不同，坐标对是视图无关标识）。
...
- **HTML**：生成**零外部依赖的自包含 HTML**——内联 SVG 快照（布局坐标 + 两行节点样式与页面一致）+ 原生 JS 滚轮缩放/拖拽平移/双击复位。无 CDN、无网络请求，隔离内网环境离线双击即开。快照取自实时画布状态（含折叠过滤、拖拽后位置、高亮描边）。
...
4. 左栏搜索表名并点击进入子图；顶部搜索字段名，选中后紫色高亮流转边并聚焦
5. 删除某个脚本，验证全表扫清理孤立表
